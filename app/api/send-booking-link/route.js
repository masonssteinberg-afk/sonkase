export async function POST(req) {
  try {
    ...
    const html =
      ...
      `<img src="https://chefsspecialsushi.com/sonakase-logo.svg" alt="Sonakase Private Dining" width="200" height="58" style="display:block;margin:0 auto;max-width:100%;border:0;" />` +
      `</div>` +
      ...
      `<p style="font-size:10px;color:rgba(245,240,232,0.3);letter-spacing:0.2em;margin:0;">bookings@sonakase.com</p>` +
      `</div></div>`;

    await resend.emails.send({
      from: "Sonakase <bookings@sonakase.com>",
      to: email,
      subject: "Your Sonakase Booking Link",
      html,
    });
